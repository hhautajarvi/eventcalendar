...
def get_events(type):
    datenow = date.today()
    if type == 0:
        sql = "SELECT E.name, E.date, E.id, I.user_id FROM users U, events E LEFT JOIN invites I ON I.user_id=:user AND I.event_id = E.id " \
            "WHERE E.visible = 1 AND U.id = E.user_id AND E.date>=:datenow AND (E.open = 1 OR (I.event_id = E.id AND I.user_id=:user) " \
            "OR E.user_id=:user) GROUP BY E.date, E.name, E.id, I.user_id ORDER BY E.date"
        result = db.session.execute(sql, {"user":session["user_id"], "datenow":datenow})
    else:
        sql = "SELECT E.name, E.date, E.id, I.user_id FROM users U, events E LEFT JOIN invites I ON I.user_id=:user AND I.event_id = E.id " \
            "WHERE E.visible = 1 AND U.id = E.user_id AND E.date>=:datenow AND (E.open = 1 OR (I.event_id = E.id AND I.user_id=:user) " \
            "OR E.user_id=:user) AND E.type=:type GROUP BY E.date, E.name, E.id, I.user_id ORDER BY E.date"
...
def add_event(name, date, description, type, open, user_id):
    try:
        ...
        "type":type, "open":open, "user_id":user_id, "visible":visible})
